...
def _initialize_weights(
    ...
    if normalized_name == "default":
        # Match the paper's Keras HeUniform initialization by default,
        # rather than Xavier -- see _kaiming_gain for why this must be
        # requested explicitly to line up with Keras's behavior.
...
def _kaiming_gain(activation: str) -> float:
    ...
    The paper initializes with Keras's ``HeUniform``, which always uses
    the ReLU-derived bound ``limit = sqrt(6 / fan_in)`` regardless of the
    layer's actual activation -- it is not activation-aware the way
    PyTorch/JAX's own Kaiming conventions typically are. To match Keras's
    behavior exactly, this returns the ReLU gain (sqrt(2)) uniformly for
    every supported activation, including tanh (previously 1.0, which
    silently produced a narrower init range than the paper for tanh runs
    -- see pytorch_mlp.py's ``_activation_to_kaiming_nonlinearity`` for the
    same fix on the PyTorch side).
