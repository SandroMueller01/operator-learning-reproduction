...
def generate_diffusion_dataset_from_config(config: ConfigDict) -> None:
    ...
    # This mirrors the structure produced by resolve_experiment_case().
    experiment_config = config["experiment"]
    grid_config = config["discretization"]["implementation"]["grid"]
    pde_config = config["pde"]
    coefficient_config = config["coefficient"]
    data_config = config["data"]
    test_config = config["testing"]["implementation"]
    ...
    test_size = int(test_config["test_size"])
    ...
    output_dir = Path(path_config["data_dir"])
    grid = DiffusionGrid(
