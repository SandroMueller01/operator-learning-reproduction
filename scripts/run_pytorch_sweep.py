"""Run a PyTorch sweep over all available training sizes."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run PyTorch training over all train_m*_seed*.npz files."
    )
    parser.add_argument(
        "--dataset",
        required=True,
    ...
    parser.add_argument(
        ...
        default="test.npz",
        help=(
            "Template for test file names. Defaults to the single shared "
            "sparse-grid test set (test.npz) used by every seed/m-value "
            "within a case (Phase 5/6/7); pass 'test_seed{seed}.npz' for "
            "legacy per-seed random test sets."
        ),
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help=(
            "Skip (m, seed) combinations that already have a row in "
            "--output, so an interrupted sweep can continue without "
            "re-running completed trainings."
        ),
    )
...
def main() -> None:
    ...
    completed = _load_completed_rows(args.output) if args.resume else set()
    ...
    for train_file, train_size, seed in train_files:
        if (train_size, seed) in completed:
            print(f"Skipping (m={train_size}, seed={seed}): already in {args.output}")
            continue
...
def _infer_problem_name(dataset_dir: Path) -> str:
    """Infer problem name from the dataset directory."""
    return dataset_dir.name


def _load_completed_rows(output_path: str) -> set[tuple[int, int]]:
    """Read an existing metrics CSV and return the set of (m_train, seed)
    rows already present, for --resume. Returns an empty set if the file
    does not exist yet."""
