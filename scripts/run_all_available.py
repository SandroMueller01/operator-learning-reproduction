...
implementation. Scope: diffusion + Navier-Stokes-Brinkman only (Boussinesq
...
Data generation (the actual mixed FEM solves) happens separately, via the
WSL 'fenics2019'/'sparsegrid' conda environments
(scripts/wsl/generate_diffusion_case.py, scripts/wsl/generate_nsb_case.py,
scripts/wsl/run_remaining_cases.sh) -- this script's own --skip-data flag
should be used once that data already exists, since _generate_datasets
here only drives the older finite-difference placeholder pipeline
(scripts/generate_data.py), not the mixed FEM one.
"""
...
DEBUG_PYTORCH_TRAIN_CONFIG = "configs/train/pytorch_fast_debug.yaml"
DEBUG_JAX_TRAIN_CONFIG = "configs/train/jax_fast_debug.yaml"
PAPER_PYTORCH_TRAIN_CONFIG = "configs/train/pytorch_paper.yaml"
PAPER_JAX_TRAIN_CONFIG = "configs/train/jax_paper.yaml"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Restrict to specific experiment names (e.g. "
            "diffusion_affine_d4 nsb_log_d8), for staged rollout / "
            "one-case-at-a-time runs instead of the full matrix."
        ),
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help=(
            "Skip (m, seed) rows already present in each sweep's output "
            "metrics CSV, so an interrupted multi-day sweep can continue "
            "across sessions without re-running completed trainings."
        ),
    )
    parser.add_argument(
        "--continue-on-error",
...
def _select_experiments(args: argparse.Namespace) -> tuple[Experiment, ...]:
    """Select experiment matrix, optionally filtered by --case-subset."""
    experiments = PAPER_EXPERIMENTS if args.paper_matrix else DEBUG_EXPERIMENTS

    if args.case_subset is None:
        return experiments

    selected_names = set(args.case_subset)
    filtered = tuple(e for e in experiments if e.name in selected_names)

    missing = selected_names - {e.name for e in experiments}
    if missing:
        raise ValueError(
            f"--case-subset names not found in the selected matrix: {sorted(missing)}"
        )
...
def _run_pytorch_sweeps(
    ...
    train_config = PAPER_PYTORCH_TRAIN_CONFIG if args.paper_matrix else DEBUG_PYTORCH_TRAIN_CONFIG
...
def _run_jax_sweeps(
    ...
    train_config = PAPER_JAX_TRAIN_CONFIG if args.paper_matrix else DEBUG_JAX_TRAIN_CONFIG
...
def _metrics_path(
    ...
    paper_matrix:
        If true, omit the "_debug" suffix (paper-spec results are not
        debug output and shouldn't be labeled as such).
    ...
    Path
        Metrics CSV path.
    """
    suffix = "" if paper_matrix else "_debug"
    return Path(
        "results/metrics/"
        f"{experiment.name}_{target}_{framework}_{model_name}{suffix}.csv"
    )
