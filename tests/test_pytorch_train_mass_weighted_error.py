"""Tests that ``train_pytorch_from_files`` actually uses the FEM
mass-matrix-weighted Bochner-norm test error (paper Appendix A.2(vi))
when a sidecar mass-matrix file is present, instead of silently falling
back to a plain unweighted relative L2 error over raw DOF values.

This wiring was missing entirely until it was caught by a sanity check on
regenerated results: ``relative_l2_error_mass_weighted`` existed and was
unit-tested (tests/test_relative_error.py) but nothing ever called it from
the training loop. These tests guard against that regression by checking
that a non-identity mass matrix on disk actually changes the reported
test error relative to the mass-matrix-free case, for an otherwise
identical, deterministic (fixed trial_seed) run.
"""
...
from ol_reproduction.training.pytorch_train import (
...
TRAIN_CONFIG = REPO_ROOT / "configs" / "train" / "pytorch_fast_debug.yaml"
...
def test_load_mass_matrix_for_target_falls_back_to_generic_file(tmp_path: Path) -> None:
    generic = sp.identity(3, format="csr")
    save_mass_matrix_npz(tmp_path / "mass_matrix.npz", generic)
...
def test_mass_matrix_presence_changes_reported_test_error(tmp_path: Path) -> None:
    """Same data, same seed, same config -- only the presence of a
    non-identity mass matrix on disk differs. The reported
    relative_test_error must differ between the two runs, proving the
    mass-weighted path is actually exercised rather than silently
    ignored."""
    ...
    # A deliberately non-identity (non-trivial) mass matrix: uniform scale
    # plus off-diagonal coupling, unlike the implicit identity used by the
    # unweighted path.
    ...
    result_with = train_pytorch_from_files(dataset_dir=with_matrix_dir, **common_kwargs)
    result_without = train_pytorch_from_files(dataset_dir=without_matrix_dir, **common_kwargs)
